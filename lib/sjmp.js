...
function createHeader(type, method, version)
{
  ...
  if (typeof type !== 'string' || type.length != 1 || ['<', '>', '=', '+', '-'].indexOf(type) == -1) {
    return null;
  }
 ...
 * @param json   {boolean|undefined} true to generate JSON instead of array.
 *
 * @returns {object|string|null}
 */
function serialize(packet, json)
{
  if (!packet || typeof packet !== 'object' || typeof packet.type !== 'string' || packet.type.length > 1 || typeof packet.id !== 'string') {
    return null;
  }

  var type = packet.type;
  var out  = [ createHeader(type, type === types.TYPE_REPLY ? packet.status : packet.method, packet.version) ];
  ...
  if (typeof packet.resource === 'string') {
    out.push(packet.resource);
  }
  else {
    return null;
  }

  if (typeof packet.id !== 'string' || packet.id.length < 4) {
    return null;
  }

  out.push(packet.id);

  if (packet.date instanceof Date) {
    packet.date = packet.date.getTime();
  }
  else if (typeof packet.date === 'string') {
    packet.date = isNaN(+packet.date) ? new Date(packet.date).getTime() : +packet.date;
  }
  else if (typeof packet.date !== 'number') {
    packet.date = 0;
  }

  out.push(packet.date);
  out.push(packet.headers || {});
  out.push(packet.body);
